...
#        listofsourceclasses =(list(list(zip(*listofrelationships))[0]))
    ...
    #@para source is the OLD source class for relationship 
    #@para destination is the OLD destination class for relationship
    #@para newsource is the NEW destination class for relationship
    #@para newdestination is the NEW destination class for relationship
    # Edit a relationship, needs to be discussed if we can create on the fly relationships. 

    def Edit_relationship(self,source: str, destination: str,newsource: str, newdestination: str):
        status=""
    # search
        sourceclass = findClass(source)
        destinationclass = findClass(destination)
        ...
        self.Delete_relationship(source, destination)
        #this will be great for debugging we can remove the Exception in prod. 
        if sourceclass is not None and destinationclass is not None:
            try:
                newrelationship = (newsource, newdestination)
                listofrelationships.append(newrelationship)
